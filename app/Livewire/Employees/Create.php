...
class Create extends Component
{
    ...
    public bool $identityFieldsEnabled = false;
    ...
    public function updatedDni(string $value): void
    {
        ...
        if (Employee::query()->where('dni', $this->dni)->exists()) {
            $this->notifyWarning('Ya existe un empleado registrado con este DNI.');
    ...
    {
        abort_unless(Auth::user()?->can('create.employee'), 403);
        ...
        try {
            DB::transaction(function () use ($validated) {
                ...
                $this->syncArea($employee, $validated['areaId'] ?? null);
            });

            $this->flashSuccess('Empleado creado correctamente.');

            $this->redirectRoute('employees.index');
        } catch (Throwable) {
            $this->notifyError('No se pudo crear el empleado. Inténtalo nuevamente.');
        }
    }
    ...
    protected function messages(): array
    {
        return [
            ...
            'dni.unique' => 'Ya existe un empleado registrado con este DNI.',
